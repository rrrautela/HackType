...
var str = `import java.util.Scanner;
...
public class MandelbrotExplorer {
    ...
    public static void main(String[] args) {
        ...
        Function<Complex, Integer> escapeTime = (c) -> {
        ...
        IntStream.range(0, height).parallel().forEach(y -> {
            double cY = ymin + (y / (double) (height - 1)) * (ymax - ymin);
            IntStream.range(0, width).forEach(x -> {
                double cX = xmin + (x / (double) (width - 1)) * (xmax - xmin);
                Complex c = new Complex(cX, cY);
                int escape = escapeTime.apply(c);
                image[y][x] = escapeTimeMap.getOrDefault(escape, ' ');
            });
        });
    ...
    private static final Map<Integer, Character> escapeTimeMap = Map.ofEntries(
            Map.entry(0, ' '),
            Map.entry(1, '.'),
            Map.entry(2, '*'),
            Map.entry(3, 'o'),
...
YEAH MATE! THE CODE ENDS HERE
HAPPY HACKING :)`



var hacktext = document.querySelector("pre");
hacktext.innerHTML = "Press any key at all as many times as u want to";
// b.innerHTML = "js running"
// b.textContent = "This is some text";

function scrollIfPageOverflows(scrollAmount = 50) {
    // Ensure compatibility with various browsers
    const pageHeight = document.documentElement.scrollHeight || document.body.scrollHeight; // Prioritize `scrollHeight` on `documentElement` for wider compatibility
    const windowHeight = window.innerHeight-150;
  
    // Calculate the actual scrollable amount (avoid unnecessary scrolling)
    const currentScrollTop = window.scrollY;
    const maxScrollableHeight = pageHeight - windowHeight;
    const desiredScrollTop = Math.min(currentScrollTop + scrollAmount, maxScrollableHeight);
  
    // Smoothly scroll only if necessary (improves user experience)
    if (desiredScrollTop > currentScrollTop) {
      ...
      console.log("Page is already scrolled to the bottom or content fits within viewport.");
    }
  }
...
document.addEventListener("keydown", function(e) {
    ...
    while(i<j){
        // if (i>0 && str.charAt(i) === "" && str.charAt(i-0) === "") {
        //     b.innerHTML += "<br>";
        //     continue;}
        hacktext.textContent += str.charAt(i);
        i++;
    } 
    j+=5;
    scrollIfPageOverflows();
